...
function Form({}: { children?: ReactNode }) {
  ...
  const { steps, currentStepIndex, step, isFirstStep, isLastStep, back, next } = MultiStep([
  ...
  function ControlBtn() {
    return (
      <div className="buttons w-full flex justify-end gap-2">
        {!isFirstStep && (
          <button className="bg-purplish-blue text-white p-3 w-28 font-bold capitalize rounded-lg" onClick={back} type="button">
            Back
          </button>
        )}
